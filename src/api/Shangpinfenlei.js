...
export default ({ config, db }) => {
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			let shangpinfenleicount = await ShangpinfenleiModel.count({
				where: {
					shangpinfenlei: req.body["shangpinfenlei"]
				}
			})
            if (shangpinfenleicount > 0) {
				toRes.session(res, -1, '商品分类已存在')
            }
