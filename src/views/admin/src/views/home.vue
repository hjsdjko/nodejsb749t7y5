...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		...
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('ershoushangpin','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"rgba(44, 55, 66,1)","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-zhangjie18" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{ershoushangpinCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>二手商品总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type4" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","background":"rebeccapurple","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		...
		<div id="ershoushangpinChart1" class="echarts2" v-if="isAuth('ershoushangpin','首页统计')"></div>
		<div id="ershoushangpinChart2" class="echarts3" v-if="isAuth('ershoushangpin','首页统计')"></div>
		<div id="ershoushangpinChart3" class="echarts4" v-if="isAuth('ershoushangpin','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getyonghuCount();
		this.yonghuChat1();
		this.getershoushangpinCount();
		this.ershoushangpinChat1();
		this.ershoushangpinChat2();
		this.ershoushangpinChat3();
	},
	methods:{
		// 词云
		...
		getershoushangpinCount() {
			this.$http({
				url: `ershoushangpin/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.ershoushangpinCount = data.data
				}
			})
		},
		ershoushangpinChat1() {
        ...
        var ershoushangpinChart1 = echarts.init(document.getElementById("ershoushangpinChart1"),'macarons');
        this.$http({
            url: "ershoushangpin/group/shangpinfenlei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                ershoushangpinChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    ershoushangpinChart1.resize();
                };
            }
        });
      })
    ...
    ershoushangpinChat2() {
      this.$nextTick(()=>{
        ...
        var ershoushangpinChart2 = echarts.init(document.getElementById("ershoushangpinChart2"),'macarons');
        this.$http({
            url: `ershoushangpin/value/shangpinmingcheng/alllimittimes`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                ershoushangpinChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    ershoushangpinChart2.resize();
                };
            }
        });
      })
    ...
    ershoushangpinChat3() {
      this.$nextTick(()=>{
        ...
        var ershoushangpinChart3 = echarts.init(document.getElementById("ershoushangpinChart3"),'macarons');
        this.$http({
            url: "ershoushangpin/group/pinpai",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                ershoushangpinChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    ershoushangpinChart3.resize();
                };
            }
        });
      })
